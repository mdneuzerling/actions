...
This workflow configures a specific version of R on a specific runner, and
recreates an environment based on a snapshot created by the `renv` package.
It then runs R CMD check via the [rcmdcheck](https://github.com/r-lib/rcmdcheck)
package.

1. You want to ensure reproducibility with a specific operating system and
version of R, and consistent package versions.
2. Your package has a lockfile created with the `renv` environment.

```{r echo = FALSE, results = "asis"}
print_yaml("check-reproducible.yaml")
```
